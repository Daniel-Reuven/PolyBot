        ...
        stage('Ansible Bot Deploy') {
            ...
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: 'jenkins-ec2-user', usernameVariable: 'ssh_user', keyFileVariable: 'privatekey')]) {
                    sh '''
                    /var/lib/jenkins/.local/bin/ansible-playbook botDeploy.yaml --extra-vars "registry_region=$REGISTRY_REGION  registry_url=$REGISTRY_URL bot_image=$current-ecr-img-name" --user=${ssh_user} -i hosts --private-key ${privatekey}
                    '''
                }
            }
        }
